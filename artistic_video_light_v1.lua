...
local function main(params)
  ...
  local img_scale = math.sqrt(firstContentImg:size(2) * firstContentImg:size(3) / (style_image:size(3) * style_image:size(2)))
        * params.style_scale
  print("Style image scale: " .. img_scale)
  ...
  --create working dir
  workingdir=string.format('%swork',params.output_folder)
  ...
  for frameIdx=params.start_number + params.continue_with - 1, params.start_number + num_images - 1 do
    ...
    if params.doedges then
        ...
        print (gmiccmd)
        os.execute(gmiccmd)
        local addedgecmd=string.format("gmic -i %s -i %s -blend %s,%s -o %s" , wcolor , wedges , params.edgesmode , params.edgesopacity , wcolor)
        print (addedgecmd)
